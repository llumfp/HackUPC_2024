...
def chatbot_assistant(query,tools_accessible):
    planning_function_applied = False
    messages = []
    ...
    if message['finish_reason'] == 'tool_calls':
        ...
        for function in message['message']['tool_calls']:
            ...
            if function_name in globals():
                ...
                if function_name == 'planning_function':
                    planning_function_applied = True
        ...
        return formatted_text, planning_function_applied
...
def save_json(dicc,file='./Files/product_list.json'):
...
def add_item(products,quantities):
    ...
    df = pd.read_csv('./Files/data_final.csv')
    ...
    df.to_csv('./Files/data_final.csv',index=False)
