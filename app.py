...
def text_to_speech(text):
    ...
    full_path = Path('Files') / speech_file_path
...
def main():
    ...
    if audio_bytes:
        ...
        # audio_file_path = max([f for f in os.listdir("./Files") if f.startswith("audio")], key=os.path.getctime)
        transcript_text = transcribe_audio(saved_file)
        ...
        df = load_json_to_df("Files/product_list.json")
        names_df = pd.read_csv('./Files/data_final.csv')
        ...
        # if is_planning:
        #     st.image('Files/simulacion_incendio.gif', caption="Planning process visualization")
